...
export default function AIInsightsPage() {
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      // Step 1: Fetch user's savings goal from database
      let userSavingsGoal: number | null = null;
      try {
        const goals = await profileApi.getFinancialGoals();
        userSavingsGoal = goals.savingsGoal || null;
        setSavingsGoal(userSavingsGoal);
        console.log("Fetched savings goal from database:", userSavingsGoal);
      } catch (err) {
        console.warn("Could not fetch savings goal:", err);
      }
      ...
      // Step 3: Call AI recommendations with REAL savings goal from database
      const result = await api.getAIRecommendations({
        ...
        transactions: transactions.map(txn => ({
          id: txn.id.toString(),
          date: txn.transactionDate || txn.date || new Date().toISOString(),
        ...
        <div className="flex items-center gap-3">
          ...
          {savingsGoal && (
            <div className="text-sm text-slate-600">
              Your savings goal: <span className="font-semibold text-emerald-600">{formatCurrency(savingsGoal)}</span>
            </div>
          )}
        </div>
        <button
